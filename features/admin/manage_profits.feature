 ...
 Функционал: Настройка стоимости действий пользователя
  ...
  Сценарий: Просмотр стоимости
    ...
    То я увижу табличные данные в ".profits_table":
      | Действия                                        | Стоимость  |
      | Находка мп3 (модуль заказов)                    | 0.01 руб.  |
      | Добавление новости (после апрува)               | 0.02 руб.  |
      | Аплоад файла (после апрува)                     | 0.10 руб.  |
      | Доход от суммы потраченной на сайте рефералом % | 40.00%     |
      | Нарезка файла (после клика на скачивание)       | -0.01 руб. |
      | Скачивание mp3 файла                            | -0.02 руб. |
      | Заказ mp3                                       | -0.02 руб. |
      | Минимальная сумма для выплаты                   | 10.00 руб. |

  Сценарий: Редактирование стоимости
    Допустим в сервисе записана стоимости по умолчанию
    Если я перешел на страницу "edit_admin_profits"
    И изменил поле стомости "amount" для "Находка мп3 (модуль заказов)" на "0.02"
    ...
    То я увижу табличные данные в ".profits_table":
      | Действия                                        | Стоимость  |
      | Добавление новости (после апрува)               | 0.01 руб.  |
      | Находка мп3 (модуль заказов)                    | 0.02 руб.  |
      | Аплоад файла (после апрува)                     | 0.05 руб.  |
      | Доход от суммы потраченной на сайте рефералом % | 30.00%     |
      | Нарезка файла (после клика на скачивание)       | -0.02 руб. |
      | Скачивание mp3 файла                            | -0.10 руб. |
      | Заказ mp3                                       | -0.50 руб. |
      | Минимальная сумма для выплаты                   | 40.00 руб. |
