module UserBalance
  ...
  def can_buy(summa)
    errors.clear
    errors.add(:base, "Недостаточно денег") unless summa <= self.balance
    errors.blank?
  end
  ...
  def can_withdraw(summa=self.balance)
    errors.clear
    errors.add(:base, "Недостаточно денег") unless summa <= self.balance
    errors.add(:base, "Сумма не должна быть меньше #{Profit.minimum_withdraw}") if summa < Profit.minimum_withdraw
    errors.blank?
  end
